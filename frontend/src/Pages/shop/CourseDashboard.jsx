...
function DashboardContent({ onLogout }) {
  ...
  const topFeedback = data?.top_feedback
    ? data.top_feedback.slice(0, 8).map((i) => ({
        ...
        count: i.count,
        full: i.text,
        is_negative: i.is_negative || false,
      }))
    : [];
...
function BarSection({ title, data, dataKey, nameKey }) {
  ...
  // Custom cell renderer for Top Feedback to color negative feedback red
  const renderBar = () => {
    if (title === "Top Feedback" && data && data.some(item => item.is_negative !== undefined)) {
      ...
      return (
        <Bar dataKey={dataKey} radius={[8, 8, 0, 0]}>
          {data.map((entry, index) => (
            <Cell 
              key={`cell-${index}`} 
              fill={entry.is_negative ? "#F87171" : "#6366F1"} 
            />
          ))}
        </Bar>
      );
    }
    // Default bar for other charts
    return <Bar dataKey={dataKey} fill="#6366F1" radius={[8, 8, 0, 0]} />;
  };
